# Spatial Regression: A Student Guide to Correlated Errors, GLS, Spatial Error Models, and SAR Models

Regression models a **mean relationship** between an outcome and predictors.
...
Spatial data frequently violate that assumption because observations close in space can have correlated unexplained variation.
...
Spatial regression becomes important when location-dependent structure remains after the mean has been modeled.

The companion script [`spatial_regression_visualizations.py`](../../scripts/spatial_statistics/spatial_regression_visualizations.py) reproduces the worked examples and generates all figures in this chapter.
...
The script creates a `assets/spatial_statistics/spatial_regression/` folder automatically.
...
Spatial regression is needed when the residual structure is not adequately represented by independent errors.

---

## 2. Why the mean must be diagnosed first

Residual spatial autocorrelation can appear because the model is missing systematic spatial structure.
...
These mechanisms are scientifically different.

If the mean is wrong, a spatial covariance term can absorb variation that should have been explained by the predictors.

![Mean misspecification](../../assets/spatial_statistics/spatial_regression/01_mean_misspecification.png)

---

## 3. Numerical example: omitted spatial trend
...
Most of the apparent spatial dependence disappeared because it was really mean structure.

---

## 4. OLS with spatially correlated errors
...
However, with non-spherical error covariance:
...
## 5. The correct covariance of the OLS estimator
...
\[
\boxed{
\operatorname{Cov}(\hat\beta_{\text{OLS}})
=
(X^\top X)^{-1}
X^\top\Sigma X
(X^\top X)^{-1}
...
## 6. What spatial correlation does to information
...
Ignoring this redundancy often makes uncertainty look too small.

This is analogous to the logic of kriging: nearby observations can be informative about the same local process, but they can also be redundant.

![Spatially correlated residuals](../../assets/spatial_statistics/spatial_regression/02_spatially_correlated_errors.png)

---

## 7. A structured covariance model

For point-referenced spatial regression, a common covariance model is
...
For nearby locations, \(d_{ij}\) is small and covariance is large.

For distant locations, the spatial covariance decreases.

---

## 8. Numerical covariance example
...
The errors are substantially correlated.
...
GLS therefore reduces to OLS.

---

## 10. What GLS is calculating
...
That is an unweighted sum of squared residuals.
...
The inverse covariance matrix adjusts the criterion for:

- unequal uncertainty;
- correlation among observations.

Highly redundant error directions receive less effective weight.

---

## 11. Whitening interpretation
...
This is often the cleanest way to understand GLS.

---

## 12. Fully worked OLS versus GLS example
...
## 14. GLS calculation with correlated errors
...
That is not a universal result. It comes from the geometry of this small example.

The important point is that GLS changes how the observations are combined by accounting for error covariance.

---

## 15. Efficiency: why GLS can estimate coefficients more precisely

Under the correctly specified covariance model, the GLS covariance is

\[
\boxed{
\operatorname{Cov}(\hat\beta_{\text{GLS}})
=
(X^\top\Sigma^{-1}X)^{-1}
}.
\]

Among linear unbiased estimators under the assumed covariance, GLS is efficient.

In repeated samples, OLS and GLS may both center on the true coefficient while GLS has a smaller sampling variance.
...
In real applications, \(\Sigma\) is almost never known.
...
Uncertainty should ideally reflect the fact that \(\Sigma\) was estimated rather than known.

The companion simulation uses the true \(\Sigma\) only as a teaching device.

---

## 17. Spatial error models for areal data

For areal data, dependence is often represented through a spatial weights matrix \(W\).
...
## 18. What the spatial error parameter means
...
A local shock in \(\varepsilon\) propagates through the error structure.
...
## 19. Numerical spatial-error example
...
## 20. Spatial lag / SAR outcome model
...
is called a spatial multiplier.

---

## 21. Why OLS on \(y\) versus \(Wy\) is generally inappropriate
...
This is an endogeneity problem.
...
SAR models require methods designed for the simultaneous spatial system, such as maximum likelihood or suitable instrumental-variable/GMM methods.

---

## 22. SAR is not just regression with a neighbor average
...
The model defines all locations simultaneously.

That changes both estimation and interpretation.

---

## 23. Direct and indirect effects in SAR models
...
Its off-diagonal entries describe spillovers to other locations.

---

## 24. Three-region SAR impact example
...
## 25. Interpret one predictor change
...
## 26. Mean structure, spatial error, or spatial lag?

A significant Moran statistic on OLS residuals tells us that residual structure remains.

It does not tell us which model to use.
...
## 27. A practical decision sequence
...
The answers narrow the appropriate model class.

---

## 28. Spatial confounding

Spatial predictors often vary smoothly over space.

Latent spatial effects also vary smoothly.

The two can therefore explain similar patterns in the outcome.
...
## 29. Why spatial confounding matters
...
This does not automatically mean the spatial model is wrong.

It means the predictor and latent spatial effect are competing to explain similar variation.

Coefficient interpretation should therefore be accompanied by sensitivity analysis.

---

## 30. A coefficient is not made causal by adding a spatial effect
...
## 31. Residual diagnostics after spatial modeling

After fitting a spatial model, diagnostics should match the model.
...
So it is often useful to inspect **whitened residuals**:
...
## 32. Residual Moran's I after GLS
...
Whitened or innovation residuals are one way to check this.

---

## 33. Residual variograms
...
Spatial dependence and heteroskedasticity are separate issues.
...
Influence diagnostics should therefore be interpreted together with spatial location.

---

## 36. Prediction and coefficient inference are different goals

A model can predict very well because location acts as a proxy for omitted processes.

That does not mean its regression coefficient has a strong scientific interpretation.
...
## 37. Random cross-validation can be too optimistic
...
Do not choose a model only because it gives the smallest in-sample residual sum of squares.

---

## 39. A complete workflow

A defensible spatial-regression analysis can proceed as follows.

### Step 1: define the spatial support
...
### Step 2: inspect the response and covariates spatially
...
### Step 3: fit and diagnose the mean structure
...
### Step 4: inspect residual spatial structure
...
### Step 5: choose a dependence model scientifically
...
### Step 6: estimate the model using appropriate methods

Do not fit an endogenous SAR lag with ordinary OLS.

### Step 7: inspect model-appropriate residuals
...
### Step 8: evaluate coefficient sensitivity
...
### Step 9: validate according to the prediction task
...
### Mistake 1: adding a spatial covariance before diagnosing the mean

The spatial term may absorb omitted trend.

### Mistake 2: saying correlated errors automatically bias OLS coefficients
...
### Mistake 3: interpreting \(\lambda\) in a spatial error model as outcome spillover

It describes dependence in disturbances.

### Mistake 4: fitting \(y\) on \(Wy\) using ordinary OLS in a SAR model

\(Wy\) is generally endogenous.

### Mistake 5: interpreting the SAR coefficient \(\beta\) like an ordinary regression slope
...
### Mistake 6: expecting raw GLS residuals to be spatially independent

The fitted covariance model explicitly allows raw residual correlation.

### Mistake 7: assuming a spatial random effect makes a coefficient causal

Causal identification requires more than residual spatial adjustment.

### Mistake 8: evaluating geographic transfer using only random cross-validation

Nearby training observations can make performance look unrealistically good.

---

## 41. Compact worked comparison
...
The coefficient may be close to the true value because the mean model is correct.
...
The main error in the naive analysis is not necessarily the point estimate.
...
## 43. Questions students should be able to answer
...
Use the companion [spatial regression exercises](../../exercises/spatial_statistics/spatial_regression.md).
