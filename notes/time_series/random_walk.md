...
$$
\operatorname{Var}(X_3\mid X_0)=3.
$$

The best conditional-mean forecast from time 3 is

$$
E[X_{3+h}\mid X_3]=X_3=4.5,
$$

for every horizon $h$. The forecast does not move, but its uncertainty grows:

$$
\operatorname{Var}(X_{3+h}\mid X_3)=h\sigma^2.
$$

![A simulated random walk](../../assets/time_series/intro_random_walk.png)

The companion white-noise/random-walk comparison is generated by [time_series_student_visualizations.py](../../scripts/time_series/time_series_student_visualizations.py).
...
The first differences recover $(1,-2,0.5,1.5)$. If the shocks have variance $\sigma^2=1$, then

$$
\operatorname{Var}(X_4\mid X_0)=4.
$$

The path can move down even when the long-run model has no deterministic downward trend. One realization should not be treated as evidence for a stable slope.

### Forecasts

With zero-drift random walk, the conditional mean forecast is flat:

$$
\hat X_{T+h|T}=X_T.
$$

For the path above at $T=4$, all point forecasts equal 6. The forecast error variance is

$$
\operatorname{Var}(X_{T+h}-\hat X_{T+h|T}\mid X_T)=h\sigma^2.
$$
...
Estimate drift cautiously: a structural break or a temporary trend can make a constant drift misleading.

### Why levels are misleading

The level ACF of a random walk often remains large over many lags, and a regression of one unrelated random walk on another can look impressive. The differenced series is closer to white noise:

$$
\nabla X_t=(1-B)X_t=\varepsilon_t.
$$

This does not mean every series should be differenced automatically. A trend-stationary process, a cointegrated system, and a deterministic seasonal pattern require different treatment.

### Extensions

- A **random walk with drift** adds a deterministic increment.
- A **local level** allows the latent level to move while observations have measurement noise.
- A **fractionally integrated** process can exhibit long memory between stationary and unit-root behavior.
- A **cointegrated system** can contain non-stationary levels whose combination is stationary.

### Diagnostics

Use a level plot, first-difference plot, rolling variance, ACF before and after differencing, and out-of-sample forecasts. A unit-root test depends on deterministic terms and may have low power near the boundary. Report the model and transformation rather than only a p-value.
